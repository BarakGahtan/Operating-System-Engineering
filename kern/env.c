...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (i = start_va; i < end_va; i += PGSIZE) {
			struct PageInfo* page_info = page_alloc(0); //not initialized
			if(page_info == NULL) panic("FAILED in region_alloc - page_alloc failure\n");
			res = page_insert(e->env_pgdir, page_info, i, PTE_P | PTE_U | PTE_W);
			if(res != 0) panic("FAILED in region_alloc - page_insert failure\n");
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	for(; prog_header < elf_prog_header; prog_header++ ) {
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	int res = env_alloc(&environment, 0);
	if(res != 0) panic("FAILED in env_create, %e\n", res);
