...
void
buffer_cache_eviction(void *addr)
{
	uint32_t block_num = ((uint32_t)addr - DISKMAP) / BLKSIZE;
	int r;

	if(addr < (void *)DISKMAP ||
...
void
eviction_wrapper(bool only_not_accessed)
{
	uint32_t i;
    uint32_t n = DISKSIZE / BLKSIZE;
	for(i = 3; i < n; ++i){
		if(!only_not_accessed || !(uvpt[PGNUM(diskaddr(i))] & PTE_A)){
			buffer_cache_eviction(diskaddr(i));
		}
	}
}
