...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if((res = page_insert(env->env_pgdir, page_info, va, perm)) != 0) {
        page_free(page_info);
        return -E_NO_MEM;
    }
    return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    if ((res = envid2env(srcenvid, &env_src, 1)) != 0) {
        return res;
    }
    if ((res = envid2env(dstenvid, &env_dst, 1)) != 0) {
        return res;
    }
    if ((page_info = page_lookup(env_src->env_pgdir, srcva, &page_table_entry)) == NULL) {
        return -E_INVAL;
    }
    if ((*page_table_entry & PTE_W) == 0 && (perm & PTE_W) == PTE_W) {
        return -E_INVAL;
    }
    if ((res = page_insert(env_dst->env_pgdir, page_info, dstva, perm)) != 0) {
        return res;
    }
    return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if ((uint32_t)srcva < UTOP) {
        ...
        if ((page_info = page_lookup(curenv->env_pgdir, srcva, &page_table_entry)) == NULL) {
            return -E_INVAL;
        }
        if ((*page_table_entry & PTE_W) == 0 && (perm & PTE_W) == PTE_W) {
            return -E_INVAL;
        }
        res = page_insert(env->env_pgdir, page_info, env->env_ipc_dstva, perm);
        if (res != 0) {
            return res;
        }
